...
#  cp ${KSDIR}/minimal-generic.ks ${DST}/isolinux/ks.cfg
  ...
  genisoimage -V ${VOLUMENAME} -A ${VOLUMENAME} -o ${OUT}/Custom-${NAME} -joliet-long -b isolinux.bin -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e images/efiboot.img -no-emul-boot -R -J -v -T isolinux/. . > /dev/null
